...
pub struct BlendInventoryManager {
...
impl BlendInventoryManager {
    pub async fn new(config: &Config, signing_key: &SigningKey) -> Result<Self> {
        ...
        db.close().unwrap();
        let db = Connection::open(db_path("liquid_asset_prices.db"))?;
        // TODO populate db prices based on coinbase mid
        db.close().unwrap();
        Ok(Self {
            rpc: client,
