...
```sh
cargo run -- --config-path "Path to config folder" --secret-key "S...."
```
...
The min_hf represents the minimum health factor of the liquidator in 9 decimals. The required_profit field is the desired profit on liquidation represented in 9 decimals.
...
```sh
docker run --name liquidation-bot --rm --it -v <"Absolute path to config folder">:/opt/liquidation-bot blend-liquidation-bot --private-key S....
```
